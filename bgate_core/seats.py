...
DEFAULT_SEATS: dict[str, dict] = {
    ...
    "art": {
        ...
        "workflow": (
            ...
            "to the anchor, the less anchor it can take. When identity is the "
            "whole job, provider='krea' with model='ideogram-3' is the one model "
            "in the catalogue with a SEPARATE character-reference field rather "
            "than a style slot doing two jobs — it bills 2.5x once references "
            "are attached, so reach for it when a cast keeps collapsing into one "
            "person, not by default.\n"
