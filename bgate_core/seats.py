...
DEFAULT_SEATS: dict[str, dict] = {
    ...
    "art": {
        ...
        "workflow": (
            ...
            "\n"
            "A 3D ASSET WITH OBVIOUS LAYERS IS PLANNED BEFORE IT IS GENERATED, "
            "NOT SPLIT SILENTLY AND NOT BUILT IN ONE PASS. Asked for a whole "
            "figure at once, a model spends one budget of attention across body, "
            "clothing, hard accessories and the text printed on them, and "
            "whatever loses that competition comes back deformed. MEASURED, on a "
            "user's baseball player: the pose read fine, the hands and the cap "
            "did not, and the team logo on the cap was scrambled.\n"
            "  a. JUDGE. A character wearing things is multi-layer. A rock is "
            "not. Single-layer subjects go straight through — this sequence is a "
            "cost, and paying it for a crate is waste.\n"
            "  b. NAME THE LAYERS AT THE LEVEL A PERSON DESCRIBES THE THING. A "
            "baseball player is body, uniform, cap, glove, cleats, team logo. "
            "SIX. Not laces, not buttons, not each sock. EIGHT IS THE CEILING; a "
            "subject that genuinely needs more is two assets, not one run that "
            "spends five hours itemising shoelaces.\n"
            "  c. DECLARE IT AND STOP. ask_human with the numbered list BEFORE "
            "one image is generated — every planned generation named, so the "
            "human keeps, cuts or rewords them one at a time. They are the only "
            "one who knows the glove matters and the socks do not, and after the "
            "spend that knowledge is worth nothing. Generate the approved layers "
            "and no others.\n"
            "  c2. BUILD EACH LAYER WITH blender_run's KIT, NOT WITH YOUR OWN "
            "HELPERS. bg_box, bg_join, bg_mirror, bg_clean, bg_unwrap, bg_mat, "
            "bg_bone_chain and bg_finish are already in scope. MEASURED: an "
            "agent wrote 33 KB of exactly these before modelling anything, then "
            "lost twenty minutes to the mesh hygiene bg_clean does in four "
            "lines. Call bg_finish last in every layer script and a layer "
            "leaves clean, applied, unwrapped and materialled.\n"
            "  c3. THE SURFACE IS GENERATED, NOT TYPED. Generate a texture per "
            "layer with image_generate conditioned on the pinned refs, then "
            "blender_texture it onto that layer BEFORE assembly. MEASURED: the "
            "first assembled character carried 21 materials and ZERO images — "
            "every surface a flat colour somebody typed, which is a blocking-in "
            "tool and not an asset. bg_mat is for blocking in; a shipped layer "
            "has an image on it.\n"
            "  d. ASSEMBLE WITH blender_combine, never by hand. A LOGO OR ANY "
            "TEXT IS ITS OWN LAYER WITH decal_on=<the surface it sits on>: baked "
            "into a body generation it comes back scrambled, and modelled flush "
            "against the surface it z-fights, which reads in-engine as the logo "
            "tearing. Shrinkwrap plus an offset is what stops both. Hard things "
            "ride a bone (bind='bone:Head' for a cap); soft things deform "
            "(bind='deform' for a jersey). Name the layer holding the armature "
            "as rig= or nothing binds and you have shipped a statue.\n"
            "  e. READ THE CHECKS IN THE RESULT. `unbound` and "
            "`unweighted_verts` name the layer that detaches or tears the first "
            "time it animates. They name a LAYER, which is the point of having "
            "them: re-run that one, not the character. `bound` also names HOW "
            "each layer weighted — deform:heat is what you want, deform:envelope "
            "is acceptable, deform:nearest is rigid and means that layer's mesh "
            "needs cleaning.\n"
            "  f. LOOK AT IT. blender_turnaround renders four angles under a "
            "rig scaled to the subject and returns a verdict per frame. OPEN "
            "THE IMAGES — that is rule 'LOOK at the frame before you call it "
            "done' and it is not optional because the asset is 3D. MEASURED: "
            "four turnarounds of a correctly-coloured model came back white, "
            "because the lights were too hot, and shipped unopened; the mesh "
            "was never the problem and an agent that had looked would have "
            "known in one second. A blown-out or black frame is a LIGHTING "
            "result — do not go back and change geometry over it.\n"
            "  g. SWEEP WHEN IT IS ACCEPTED. blender_sweep removes that run's "
            "per-layer intermediates and keeps the asset, the renders and the "
            "manifest, so the record of what was built from what survives the "
            "cleanup. Dry-run first; never delete a layer file by hand, because "
            "the manifest is what makes a single layer re-runnable later."
        ),
    },
    "audio": {
        "title": "Audio",
