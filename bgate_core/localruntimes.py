...
WHY THIS EXISTS. ``bgate_core.providers`` gave the hosted providers a real
...
WHY THIS IS NOT :class:`bgate_core.providers.Provider`, AND THE TWO SIT SIDE BY
...
``bgate_core.envfile``, exactly like a provider key and for a reason that is not
symmetry.
...
from bgate_core import comfyui, envfile
from bgate_core.providers import CAPABILITIES
...
class Runtime:
    ...
    start: tuple[str, ...]
    docs_url: str = ""
    # "comfy" means the introspection in bgate_core.comfyui applies.
    software: str = ""
