...
[`.github/workflows/ci.yml`](workflows/ci.yml). Two you run directly:
...
That writes `src/bgate_ui/static/`. **Do not hand-edit anything under
`src/bgate_ui/static/`** — it is build output, and the next build overwrites it.
...
A fresh clone has no `src/bgate_ui/static/` beyond `dist/`, because only the bundle
...
That means the built front end under `src/bgate_ui/static/`, anything in
`src/templates/`, or the engine schemas. A wheel that quietly ships no JavaScript is
...
`src/templates/`, and serves the dashboard's assets out of `site-packages`.
