...
from bgate_ui.agents import autodeploy, dispatch as _dispatch, qa_gate
...
class TestApproveAndReject:
    def test_approval_closes_it_and_records_who(self, root, monkeypatch):
        ...
        got = queue.approve(root, item["id"], note="ship it", by="marta")
        assert got["status"] == "done"
        assert got["approved_by"] == "marta"
        assert "APPROVED by marta" in got["result"] and "ship it" in got["result"]
