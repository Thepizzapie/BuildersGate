...
from bgate_core.store import db, events, settings
from bgate_core.board import gates, queue
from bgate_ui.pumps import heartbeat
...
class TestStall:
    ...
    def test_movement_re_arms_the_reminder(self, root, held):
        ...
        _age(root, first["id"], 240)
        heartbeat.tick(root)
        queue.approve(root, first["id"], by="marta")     # it moved
    ...
    def test_a_finished_chain_drops_its_mark(self, root, held):
        ...
        _age(root, first["id"], 240)
        heartbeat.tick(root)
        queue.approve(root, first["id"], by="marta")
