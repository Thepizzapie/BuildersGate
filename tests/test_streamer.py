...
def filt():
    ...
    return streamer.Redactor(
        home=r"C:\Users\adria", user="adria", host="adria-desktop",
        roots=[r"C:\Users\adria\Desktop\downsizing"],
...
class TestIdentity:
    def test_the_home_directory_goes(self, filt):
        out = filt.text(r"wrote C:\Users\adria\Desktop\notes.txt")
        assert "adria" not in out
    ...
    def test_the_project_root_wins_over_home(self, filt):
        ...
        out = filt.text(r"C:\Users\adria\Desktop\downsizing\game.tscn")
    ...
    def test_forward_slashes(self, filt):
        ...
        assert "adria" not in filt.text("C:/Users/adria/Desktop/downsizing/x.gd")
    ...
    def test_json_escaped(self, filt):
        ...
        blob = json.dumps({"root": r"C:\Users\adria\Desktop\downsizing"})
        assert "adria" not in filt.text(blob)

    def test_percent_encoded(self, filt):
        """The preview endpoint takes paths in a query string."""
        assert "adria" not in filt.text("/api/preview?path=C:%5CUsers%5Cadria%5Cx.png")
    ...
    def test_case_insensitive(self, filt):
        ...
        assert "Adria" not in filt.text(r"c:\users\Adria\Desktop\a.txt")
    ...
    def test_the_bare_username(self, filt):
        assert "adria" not in filt.text("git author: adria pushed 3 commits")

    def test_the_hostname(self, filt):
        """Usually the owner's name plus a device."""
        assert "adria-desktop" not in filt.text("host=adria-desktop ok")
...
class TestSecrets:
    ...
    def test_credentials_in_a_url(self, filt):
        out = filt.text("https://adria:hunter2@github.com/x/y.git")
        assert "hunter2" not in out
        assert "adria" not in out, "the username half doxes too"
...
class TestRoundTrip:
    def test_a_path_survives_out_and_back(self, filt):
        ...
        real = r"C:\Users\adria\Desktop\downsizing\assets\hero.glb"
        assert filt.restore(filt.text(real)) == real

    def test_nested_structures(self, filt):
        payload = {"root": r"C:\Users\adria\Desktop\downsizing",
                   "steps": [{"cmd": r"blender C:\Users\adria\x.blend"}]}
        out = filt.scrub(payload)
        assert "adria" not in json.dumps(out)
        assert filt.restore(out)["root"] == r"C:\Users\adria\Desktop\downsizing"
    ...
    def test_dict_keys_are_scrubbed(self, filt):
        ...
        out = filt.scrub({r"C:\Users\adria\Desktop\downsizing": {"ok": True}})
        assert "adria" not in json.dumps(out)
