...
def test_the_published_index_carries_no_local_paths(tmp_path, game):
    ...
    assert report["games"][0]["root"] == str(game)      # still local
    text = (out / "games.json").read_text(encoding="utf-8")
    published = json.loads(text)["games"]
    assert published and all("root" not in g for g in published)
    assert str(game) not in text
    assert str(game) not in (out / "index.html").read_text(encoding="utf-8")
