...
Three kinds of document live here. **Onboarding** is for someone arriving
without context. **Reference** is the detail the README links out to.
**Findings** are write-ups of what went wrong on real production runs, kept
...
| [sprite-animation-research.md](sprite-animation-research.md) | Why sheets that passed every gate still animated badly. The registration bug (bounding boxes are not bodies), palette locking versus palette detection, per-frame timing Godot had all along, and the four cross-frame faults an identity judge cannot see. |
| [cinematic-research.md](cinematic-research.md) | The video-model landscape, the Godot format wall that silently swallows a finished cutscene, why cutscenes are an eighth seat rather than a wider art seat, and the post-production half: transitions, score, captions, continuity. |
...
| [decisions/](decisions/) | The full narrative behind every release, one file per version, moved verbatim out of `CHANGELOG.md` when that file was cut to summaries. |
