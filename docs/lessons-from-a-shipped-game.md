...
| --- | --- |
| Four seat-mission sentences (L1, L2, L16, L18), verified in `src/bgate_core/board/seats.py` | `asset_orphans` (L4) |
| `src/templates/shared/CLAUDE.md`: chain decay, derivation, fixtures, determinism, stale legends, leftovers (L3, L5, L6, L8, L9) | `content_determinism_check` (L1) |
| Scene composition and render-before-done, already ported before this pass | `bible_add(acceptance=...)` (L11) |
| | `src/templates/shared/tests/` and `design/_spec.md` (L10, L21) |
...
loader validation or scaffold stamp · **TEMPLATE** = `src/templates/` ·
...
| L3 | Pin a fixture to what is **structurally guaranteed**, never to whatever is least finished today. | RULE | `src/templates/shared/CLAUDE.md` (done) | `game/tests/dialogue_test.gd:190-202` (fixture moved twice, both times because art got made), `:377-380`; `tests/testbed_tests.gd:10-17` |
...
| L5 | **Generate the minimum, derive the rest.** Generation is spent only on genuinely new silhouettes. | RULE | `src/templates/shared/CLAUDE.md` art pipeline (done) | `tools/spritekit.py:22-34`, `:186-194`; `build_character.py:14-17`; `gen_weapons.py:15-17`; `derive_wall_tiles.py:14-21`; bible §14/§15 (rotation classes: 1, 2 or 4 gens, declared at ticket time) |
| L6 | **A generation chain decays.** Never condition frame N on frame N-1. | RULE | `src/templates/shared/CLAUDE.md` art pipeline (done) | `tools/gen_actions.py:3-19` (by frame 3 the character turned front-facing and shrank 932px to 821px); `gen_idles.py:5-12`; `spritekit.py:186-194`, `:236-241` |
...
| L8 | Integration lines you do **not** own get written down verbatim at a named call site, not half-landed. | RULE + TEMPLATE | `src/templates/shared/CLAUDE.md` "Leftovers" (done) | `scripts/sim/exploration.gd:48-72`, `loot.gd:63-85`, `inventory.gd:53-77`, `shop.gd:86`, `combat.gd:462`, `skills.gd:1027` (6 of 9 sim files); `design_exploration.md:423-436`; `tests/exploration_test.gd:578` |
| L9 | A preview whose **legend can go stale lies more confidently** than raw data does. | RULE | `src/templates/shared/CLAUDE.md` (done) | `tools/render_floor.py:38-43` (renderer drew every desk as an enemy), `:84-90`, `:102-105`; `design_set_dressing.md:288-291` |
| L10 | A spec states **what it did not do and what is still dark**. "Flagged, not fixed" beats a silent gap. | TEMPLATE | `src/templates/shared/design/_spec.md` | Present in all 17 docs: `design_story_arc.md:706`; `design_equipment.md:661`; `design_skill_trees.md:562`; `design_exploration.md:502`; `design_loot.md:489-491` |
...
| L21 | The house test contract: `PASS <name>` / `FAIL <name>: <why>` / `QA_RESULT pass=N fail=N`, non-zero exit. | TEMPLATE | `src/templates/shared/tests/` harness stub | `game/tests/loot_test.gd:19-25`, `:56`, identical in all 10 harnesses; `tests/exploration_test.gd:5-7`; fold-in idiom at `tests/combat_test.gd:609-630`; copyable boilerplate at `campaign_test.gd:11-15` |
| L22 | Price the option you are actually **testing**, and report quality-per-dollar. | CHECK | `src/bgate_core/board/spend.py` (mostly covered) | `tools/bakeoff.py:31-37` (ideogram charges 2.5x for exactly the field being tested), `:70-94` |
...
`asset_verify` (`src/bgate_core/store/assets.py:473-511`) compares tracked assets to disk by
...
cheaper move may be to stamp the loader guard into `src/templates/shared/` so a new
...
- **`src/templates/shared/tests/`** (L21). One `SceneTree` harness with the
  ...
  `src/bgate_core/store/scaffold.py:66-83` already overlays `shared/` wholesale, so this is
  a file drop.
- **`src/templates/shared/design/_spec.md`** (L10). Status / Grounded in / Scope /
...
3. **`src/templates/shared/tests/` plus the spec skeleton** (L21, L10). Both are
