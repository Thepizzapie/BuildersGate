...
NOT ``bgate_core.board.streamer``. That module is the REDACTION filter — it takes the
...
1. NOTHING HERE REACHES THE QUEUE. This module has no import of
   :mod:`bgate_core.board.queue` and no path to one. Chat becomes, at most, text in a
...
class Platform:
    ...
    ``provider_id`` points at the entry in :mod:`bgate_core.runtime.providers` that owns
...
def config(root: Optional[str | os.PathLike[str]] = None,
    ...
    Reads the project's ``.env`` through :mod:`bgate_core.store.envfile` first, so a
    ...
    :mod:`bgate_core.runtime.providers`.
    """
...
class ChatMessage:
    ...
    ``bgate_core.qa.chatfeedback.retract``.
    """
...
class Transport:
    ...
    other daemon loops in it already (see :mod:`bgate_ui.pumps.pump`), and a socket
