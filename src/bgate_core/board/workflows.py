...
# 'tool' is the generic executor added in bgate_core.board.wfnodes: a node that calls
...
def _queue_step(root: str | os.PathLike[str], run: dict, snapshot: dict,
    ...
    if dispatch:
        try:
            from bgate_ui.agents import dispatch as _dispatch  # lazy: core must not need the UI
...
def _context_output(root: str | os.PathLike[str], spec: dict) -> dict:
    ...
    (bgate_core.art.artdirection), but that is a floor, not a way to SAY THINGS. A
    ...
    if node_type == "input.bible":
        from ..design import bible as _bible
        ...
        from ..design import lore as _lore
...
def _resolve_ref_source(root: str | os.PathLike[str], name: str) -> str:
    ...
    try:
        from ..store import artifacts as _a
...
def _node_spends(spec: dict) -> bool:
    ...
    node IS an image provider call by definition (bgate_core.board.generate does
...
# than slow. Ten minutes is past every generation timeout in bgate_core.board.generate.
SWEEP_RUN_LIMIT = 20
STALL_MINUTES = 10
