...
# — and kie fills it now, through bgate_core.cine.cinematic. Note that a video key is
...
def _probe_twitch(root: Any = None) -> dict:
    ...
    ``bgate_core.qa.chatlink`` — so what makes this provider usable is the CHANNEL,
...
def routing(root: Optional[str | os.PathLike[str]] = None) -> dict:
    ...
    the pick come from :mod:`bgate_core.runtime.gateway` (which owns doctrine order,
    ...
    return {"ok": True, "reason": "", "families": families,
            ...
            "note": "families come from bgate_core.runtime.gateway - the same table "
