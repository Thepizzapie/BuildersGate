...
:func:`bgate_core.board.queue.ready`, THE one copy of the readiness rule, so a held
...
from ..store import events as _events, workspace as _ws
...
def stage(root: str | os.PathLike[str]) -> str:
    ...
    * a board with work already past 'queued' is a project that was building
      ...
      :mod:`bgate_core.board.gates` refuses to make about sign-off, and for the same
      reason.
...
def presentation_check(root: str | os.PathLike[str]) -> dict:
    ...
    * asset scale — every delivered art asset measured against the scale
      ...
      (:mod:`bgate_core.three_d.scalecontract`)
    * audio — cues reviewed against a gameplay capture, not just file metrics
    ...
    :mod:`bgate_core.board.findings`. Rows are recorded to the findings ledger with
...
def _default_scene_unmet(root) -> list[dict]:
    from ..level import sceneproof as _proof
...
def _rooms_unmet(root) -> list[dict]:
    from ..board import findings as _findings
    from ..level import roomqa as _roomqa
...
def _scale_unmet(root) -> list[dict]:
    from ..three_d import scalecontract as _scale
