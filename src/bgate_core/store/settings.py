...
#
# EVERY SETTING MUST APPEAR HERE. tests/store/test_settings_labels.py fails on a key
...
SETTINGS: tuple[Setting, ...] = (
    ...
    Setting(
        ...
        # conversation lives in bgate_ui.agents.runners, which this module may not
        ...
        if _activity.is_machine(who):
            raise SettingError(
                ...
                + " is a constraint on agents; an agent that can change it is "
                  ...
                  "bgate_core.store.settings refuses the same way.")
...
# actually route. Served by bgate_core.runtime.modelcatalog, filtered to configured
...
def _choices_for(root, s: Setting) -> list:
    ...
    try:
        from ..runtime import modelcatalog
