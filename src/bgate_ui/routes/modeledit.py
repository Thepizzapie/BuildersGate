...
# ---------------------------------------------------------------------------
# The live Blender, beside the viewer
# ---------------------------------------------------------------------------
# blender.run_script launches a fresh headless Blender per call: cold start,
# empty scene, nothing survives. blender_live talks instead to a Blender that
# STAYS OPEN, so the scene persists and the viewport a person is looking at is
# the one being built in. It has existed as four MCP tools and was absent from
# the dashboard entirely, which meant the surface holding the model could
# launch a cold Blender and forget about it, and nothing more.
#
# WHAT THIS IS AND IS NOT. It is a bridge to a Blender the user started, on
# loopback, on a port they set in that add-on's own preferences. It is not a
# sandbox: /live/run executes Python inside their Blender, which is what the
# feature IS. That is the same authority the MCP tool already has and the same
# authority any script they open has, and it is reachable only through the
# app's same-origin + bearer-token guard on mutations. The size cap below is
# there to keep a mistake small, not to make the endpoint safe to expose.
LIVE_VIEW_DIRNAME = "live_view"
LIVE_MAX_SCRIPT = 64 * 1024
LIVE_TIMEOUT = 300


def _live():
    """Imported at call time. The adapter opens a socket on first use and the
    dashboard has to serve every other route on a machine with no Blender."""
    from bgate_adapters import blender_live as mod
    return mod


def _live_port(payload: dict) -> int:
    port = _number(payload if isinstance(payload, dict) else {}, "port", 9876)
    if not 1 <= port <= 65535:
        raise api.bad_request("port must be between 1 and 65535", port=port)
    return port


@router.get("/api/model3d/live")
def model_live_status(port: int = 9876) -> dict:
    """Is a live Blender answering on that port, and does it hold the kit?

    Never an error: "nothing is listening" is the normal state on a machine
    where the add-on is not running, and a 503 for it would make the panel
    look broken rather than idle.
    """
    if not 1 <= port <= 65535:
        raise api.bad_request("port must be between 1 and 65535", port=port)
    try:
        probe = _live().available(port=port)
    except Exception as exc:
        probe = {"available": False, "port": port,
                 "error": f"{type(exc).__name__}: {exc}"}
    return api.ok({**probe, "default_port": 9876})


@router.post("/api/model3d/live/run")
def model_live_run(payload: dict) -> dict:
    """Run a kit script in the live Blender. The scene persists between calls.

    `bg_wipe()` is NOT called first: the whole point of a live session is that
    what is there stays there.
    """
    script = payload.get("script")
    if not isinstance(script, str) or not script.strip():
        raise api.bad_request("script must be a non-empty string")
    if len(script.encode("utf-8")) > LIVE_MAX_SCRIPT:
        raise api.bad_request(
            f"script is larger than {LIVE_MAX_SCRIPT // 1024} KiB",
            bytes=len(script.encode("utf-8")))
    port = _live_port(payload)
    result = _live().run(script, report=True, port=port, timeout=LIVE_TIMEOUT)
    if not result.get("ok"):
        raise api.ApiError(502, result.get("error") or "the live Blender refused",
                           detail={k: result.get(k) for k in
                                   ("error", "traceback", "live", "stdout")})
    return api.ok({**result, "port": port})


@router.post("/api/model3d/live/import")
def model_live_import(payload: dict) -> dict:
    """Push the model the viewer has open INTO the live Blender.

    THE DIRECTION THAT MAKES THE BRIDGE WORTH WIRING. Every other route here
    reads from the live scene; this one puts the thing being looked at in
    front of the person who can fix it, with their own tools, without a file
    dialogue and without them having to know where .bgate_out is.

    The path is a FILENAME and filenames are not trusted input: models arrive
    downloaded from providers and generated by agents, and a single quote in
    one ends a raw string literal and runs the rest as Python inside Blender.
    It goes through repr(), the same fix model_open_in_blender carries.
    """
    rel = str(payload.get("rel") or "")
    _, target = _model(rel)
    _importable(target)
    ...
    script = (
        ...
        "result = {'imported': after, 'count': len(after)}\n")
    result = _live().run(script, report=True, port=port, timeout=LIVE_TIMEOUT)
    if not result.get("ok"):
        raise api.ApiError(502, result.get("error") or "the import failed",
                           detail={"error": result.get("error"),
                                   "traceback": result.get("traceback")})
    return api.ok({"rel": rel, "port": port, "wiped": bool(payload.get("wipe")),
                   **result})


@router.post("/api/model3d/live/export")
def model_live_export(payload: dict) -> dict:
    """Bring the live scene back as a .glb inside the project.

    The other half of the round trip: fix it in Blender, land it here, open it
    in the viewer, run the gates over it. Writes where the caller says, inside
    the project and nowhere else.
    """
    rel = str(payload.get("rel") or "")
    base = root()
    if not rel:
        raise api.bad_request("rel must say where the .glb goes")
    out = safe_under(base, rel)
    if out.suffix.lower() != ".glb":
        raise api.bad_request("the live export writes a .glb", rel=rel)
    port = _live_port(payload)
    out.parent.mkdir(parents=True, exist_ok=True)
    result = _live().export_glb(str(out), port=port, timeout=LIVE_TIMEOUT)
    if not result.get("ok") or not out.is_file():
        try:
            out.unlink()
        except OSError:
            pass
        raise api.ApiError(502, result.get("error") or "the live export wrote nothing",
                           detail={"error": result.get("error")})
    _inspect_cache.clear()
    return api.ok({"rel": out.relative_to(base).as_posix(), "port": port,
                   "bytes": out.stat().st_size,
                   "scene": result.get("scene"),
                   "export": result.get("export")})


@router.post("/api/model3d/live/view")
def model_live_view(payload: dict) -> dict:
    """Photograph the live viewport — what the human is actually looking at.

    Under .bgate_out with the other evidence, and returned as an /api/preview
    URL so the panel can just show it. Worth having next to the model: it is
    the only way to see, from here, that the Blender being driven is showing
    what the driver thinks it is.
    """
    project_root = root()
    port = _live_port(payload)
    out_dir = project_root / ".bgate_out" / LIVE_VIEW_DIRNAME
    out_dir.mkdir(parents=True, exist_ok=True)
    shot = out_dir / f"live-{int(time.time())}.png"
    result = _live().view(str(shot), port=port, timeout=LIVE_TIMEOUT)
    if not result.get("ok") or not shot.is_file():
        raise api.ApiError(502, result.get("error") or "the live view captured nothing",
                           detail={"error": result.get("error")})
    return api.ok({"port": port,
                   "rel": shot.relative_to(project_root).as_posix(),
                   "url": "/api/preview?rel=" +
                          shot.relative_to(project_root).as_posix(),
                   "bytes": shot.stat().st_size, "view": result.get("view")})


@router.post("/api/model3d/live/reset")
def model_live_reset(payload: dict) -> dict:
    """Empty the live scene. The one destructive call, and it is named."""
    port = _live_port(payload)
    result = _live().reset(port=port, timeout=LIVE_TIMEOUT)
    if not result.get("ok"):
        raise api.ApiError(502, result.get("error") or "the reset failed",
                           detail={"error": result.get("error")})
    return api.ok({"port": port, **result})
