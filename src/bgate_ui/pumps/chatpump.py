...
— see :mod:`bgate_ui.pumps.pump` for the shape those three share. This one does not
use :class:`~bgate_ui.pumps.pump.Pump` and the difference is worth stating: those loops
...
from bgate_core.qa import chatfeedback, chatlink, feedback, playtest
...
# ---------------------------------------------------------------------------
# One link per project. Same reasoning as bgate_ui.pumps.pump's per-root latch: the
...
def redaction_advice(root: str | os.PathLike[str]) -> dict:
    ...
    they have set visible in it. ``bgate_core.board.streamer`` exists for exactly that
