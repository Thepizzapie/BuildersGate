...
the concept refs. The QA seat persona (bgate_core.board.seats, "qa") knows how to be
...
nothing said so. ``bgate_ui.agents.followup`` now drives the gate from the event bus'
...
- Disable with BGATE_QA_GATE=0, which is the legacy kill switch and now reads
  through ``bgate_core.store.settings`` as a coercion of ``gate.mode`` to "none".
"""
...
def _scan_once(root: str, cutoff_utc: str) -> None:
    ...
    NOT the normal path any more — ``bgate_ui.agents.followup`` routes completions off
