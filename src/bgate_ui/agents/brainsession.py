...
ALMOST NOTHING HERE IMPORTS bgate_ui.agents.dispatch, on purpose. The two modules
...
from bgate_ui.agents.dispatch import _kill_tree, _scrubbed_environ
...
# (bgate_core.design.brainstorm.invite) is a second process in the same room: its own
...
def stop_session(root, session_id: int) -> dict:
    ...
    session, and their state is bgate_core.design.brainstorm's to move.
