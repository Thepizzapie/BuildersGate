...
So this is ONE subscriber on ONE cursor over ``bgate_core.store.events``, and it
...
# The source stamped on a failure escalation (defined in bgate_core.board.queue, which
...
def _do_reopen(root, action: dict) -> dict:
    ...
    try:
        from bgate_core.board import salvage as _salvage
...
def tick(root: str | os.PathLike[str]) -> dict:
    ...
    count = _bump(root)
    try:
        from bgate_ui.pumps import heartbeat as _heartbeat
    ...
    if count % SWEEP_EVERY == 0:
        ...
        try:
            from bgate_core.board import workflows as _workflows
