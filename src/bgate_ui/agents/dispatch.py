...
# SEAT HOUSE RULES LIVE IN bgate_core.board.seats (DISPATCH_RULES / dispatch_rules)
...
def _persona_line(root: str, seat: str) -> str:
    ...
    """
    try:
        from bgate_core.board import seats as _s
...
_ENV_PASSTHROUGH: tuple[str, ...] = (
    ...
    # from bgate_core.runtime.providers below, so a provider added there is not
...
def _provider_env_vars() -> tuple[str, ...]:
    ...
    """
    try:
        from bgate_core.runtime.providers import PROVIDERS
    except Exception:
        return ()
...
def _auto_commit(root: str, item_id: int, entry: dict) -> None:
    ...
    (bgate_core.store.provenance): what the hook observed this owner write, what a
...
def kill_all(root: str, *, reason: str = "", actor: str = "") -> dict:
    ...
    thinking = []
    try:
        from bgate_ui.agents import brainsession as _brainsession
