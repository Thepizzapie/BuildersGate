...
from bgate_core.art import spritekit as _kit
...
def from_pose_images(pose_files: list[tuple[str, str]], *, out_dir: str,
    ...
    emitter (see :func:`_sprite_frames_tres`); ``bgate_core.art.animspec`` produces
...
def _sprite_frames_tres(sheet_filename: str, anims: list[tuple[str, int]],
    ...
    every key optional.
      ...
      * ``holds`` are Godot's per-frame relative ``duration`` — a frame at 2.0
        ...
        should hold and the wind-up should be quick; see bgate_core.art.animspec,
