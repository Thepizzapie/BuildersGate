...
_LEVEL_RULE = (
    ...
    "INPUT. run, jump_speed and gravity are in cells per second and "
    "every pit and pipe is sized from what they allow - read them from "
    "the player scene and pass the SAME numbers, because a level built "
    "for one jump and played with another is the failure this exists to "
    "prevent. It refuses an unplayable level (reachable, clearance, "
...
SEAT_RULES = {
    ...
    "art": (
        ...
        "step in a room's outline. The 47 corner tiles are built "
        "geometrically from the same generation, so eight bits costs no "
        "extra call and no extra money.\n"
