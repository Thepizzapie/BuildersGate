...
def model_open_in_blender(payload: dict) -> dict:
    ...
    resolver doctor's blender row probes through (bgate_core/doctor.py:
    ...
    if suffix == ".blend":
        ...
        if suffix in (".glb", ".gltf"):
            call = f"bpy.ops.import_scene.gltf(filepath=r'{path}')"
        elif suffix == ".obj":
            call = (f"\n try:\n  bpy.ops.wm.obj_import(filepath=r'{path}')\n"
                    f" except AttributeError:\n"
                    f"  bpy.ops.import_scene.obj(filepath=r'{path}')")
        elif suffix == ".fbx":
            call = f"bpy.ops.import_scene.fbx(filepath=r'{path}')"
        else:
