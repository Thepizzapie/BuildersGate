"""Level generation MCP tools - carved out of server.py, verbatim.
...
from bgate_mcp.server import (  # noqa: F401
    Optional, _Path, _archive_preview, _artdirection,
    _ase_master_for, _contained_path, _fail, _gameview,
    _godot, _json, _log, _note_tool_write,
    _paid_gate, _re, _res_pair, _root,
    _run_tag, _scenewire, _spritekit, _tool,
)
...
# _res_pair moved BACK to server with the rest of the shared plumbing: it is
# the containment chokepoint for every scene-addressing tool, the scene tools
# still living in server call it, and a copy here would be a second gate that
# tests (which monkeypatch server._contained_path) could not see through.
