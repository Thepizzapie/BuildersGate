...
window.Atlas = (() => {
  ...
  function _composer(seat, title, brief){
    /* THE FORM THIS FILLED IN NO LONGER EXISTS. #q-seat / #q-title / #q-brief
       were the old Agents board's queue composer; the console is React now
       (frontend/src/shell/agents/) and its composer is one text box you talk to
       the director through. Every lookup below was null-guarded, so this became
       a button that navigated you to another screen and then did nothing at
       all — no form, no toast, no console line.

       BGCompose is the console's own hook, registered while it is mounted. If
       it is not there (an older bundle, or the deck never mounted) say so
       rather than pretending: a dead deploy button is how a task quietly never
       gets filed. */
    setWorkspace("agents");
    const compose = window.BGCompose;
    if (typeof compose === "function"){
      compose({ seat: seat, title: title, brief: brief });
      return;
    }
    if (window.toast) toast("the console did not take that — copy it in by hand");
  }
