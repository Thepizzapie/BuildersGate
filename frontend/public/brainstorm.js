...
`;
  ...
  var Brainstorm = {
    ...
    /**
     * THE DRAWING PAD ON ITS OWN, IN SOMEBODY ELSE'S LAYOUT.
     *
     *   var pad = Brainstorm.mountPad(el, { sessionId: 7 });
     *   pad.destroy();
     *
     * WHY THIS EXISTS. The React room drew a thumbnail of the scene with a
     * "draw" button that opened the whole workspace over the page — so editing
     * two boxes meant leaving the room, and the owner's verdict was that the
     * pad should be usable where it is shown. The workspace itself cannot go in
     * a 320px column: it is a conversation, a writing pad AND a drawing pad.
     * The pad alone can.
     *
     * NOT A CUT-DOWN WORKSPACE. `mount({pads:false})` used to exist for exactly
     * this kind of request and was removed, because a variant that renders less
     * of the same thing is how one module quietly becomes two. This returns the
     * SAME Pad object the workspace uses, loading and saving through the same
     * API calls; there is no second implementation of the surface, the scene
     * format or the save.
     *
     * It does not poll. The pad is the human's own surface and the partner
     * writing to it mid-stroke is exactly what `adoptDrawing` guards against in
     * the workspace; a caller that wants the partner's edits can call
     * `pad.load(scene)` when its own poll brings one.
     */
    mountPad: function (host, opts) {
      if (!host) return null;
      opts = opts || {};
      var id = opts.sessionId;
      var timer = null, dead = false;
      var pad = new Pad(host, function () {
        // Debounced like the workspace's own drawing save, and for the same
        // reason: a stroke is fifty change events and set_drawing is a
        // whole-document write.
        clearTimeout(timer);
        timer = setTimeout(function () {
          if (dead || !id) return;
          API.patch(id, { drawing: pad.scene })
             .then(function (s) { if (opts.onSave) opts.onSave(s); })
             .catch(function (e) { if (opts.onError) opts.onError(e); });
        }, 1100);
      });
      if (id) {
        API.read(id).then(function (s) {
          if (dead) return;
          pad.load(s ? s.drawing : null);
        }).catch(function () { /* an unreadable session leaves an empty pad */ });
      }
      var destroy = pad.destroy.bind(pad);
      pad.destroy = function () { dead = true; clearTimeout(timer); destroy(); };
      return pad;
    },
    get active() { return ACTIVE; },

    /* THE VOICE CLIENT, FOR THE REACT ROOM.
     *
     * Voice and Mic were module-private because this workspace was the only
     * screen with a microphone on it. It is not any more: the seat's Brainstorm
     * tab and the standalone room are shell/brainstorm/Room.tsx, and they need
     * the same mic and the same speaker.
     *
     * EXPORTED RATHER THAN REIMPLEMENTED. Mic owns a MediaStream, an
     * AudioContext, a resampler, the websocket handshake that sends the
     * dashboard token as its FIRST FRAME rather than in a query string, and an
     * idempotent stop() that four different callers race into. A second client
     * for one relay would be a second place for all of that to be subtly wrong,
     * and the one that got it wrong would be the one nobody tested — while the
     * failure mode is a hot microphone the user cannot find the button for.
     *
     * `Voice.load()` must be awaited before `speak` or a `Mic`: both read the
     * server's audio contract (encoding, sample rate, the speak cap) out of
     * Voice.status rather than assuming it.
     */
    voice: Voice,
    Mic: Mic,
