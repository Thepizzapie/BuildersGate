...
(function () {
  ...
  /* WHICH TEMPLATE CATEGORIES BELONG TO WHICH SEAT.
   *
   * Keyed on the seat's own role, valued with the categories that seat's craft
   * actually produces — so the Generate tab in a workspace offers that seat's
   * generation workflows and nothing it has no lane for. A seat missing here
   * falls through to the whole library rather than to an empty panel: a new
   * seat should look unscoped, not broken.
   *
   * art owns the still-image and model pipelines (concept, anchor, animation,
   * comparison, 3D, items, world backgrounds); audio owns music and voice;
   * video owns the cutscene. `agent`, `engine` and `level` are deliberately
   * unassigned — they cross seats, and the Studio screen is where they live. */
  const SEAT_CATS = {
    art:   ["asset", "3d", "items", "world"],
    audio: ["audio"],
    cinematic: ["video"],
  };
  ...
  const WF = {
    ...
    /* The seat→categories rule, as an accessor. wf_steps_templates.js replaces
       _renderLibrary wholesale, so it needs to read this rule rather than own a
       copy — two copies is how a seat gains a family in one render and not the
       other. Null means unscoped, which is what Studio passes. */
    seatCats(seat) { return SEAT_CATS[seat || ""] || null; },

    /* THE LIBRARY BODY IN *THIS* MOUNT, NOT THE FIRST ONE IN THE DOCUMENT.
     *
     * getElementById is a document-wide lookup, and this app keeps its decks
     * mounted and hidden — so once a seat's Generate tab has opened the
     * library, TWO #wf-lib-body elements exist and the id resolves to Studio's
     * hidden one. Every render then landed in the invisible copy and the seat
     * sat on "loading…" forever. Scoped to the host open() was handed, with the
     * document as the fallback for the Studio case that predates a host. */
    _libBody() {
      if (this._host) {
        const scoped = this._host.querySelector("#wf-lib-body");
        if (scoped) return scoped;
      }
      return document.getElementById("wf-lib-body");
    },
    ...
    /* ---- library landing ------------------------------------------------ */
    /* `opts.seat` SCOPES THE LIBRARY TO ONE CRAFT.
     *
     * The seat workspaces mount this in a Generate tab, and a video seat that
     * offers "Level layout" and "Scene surgery" is a menu whose entries mostly
     * belong to somebody else. Scoping is a FILTER over the same registry, not
     * a second library: a template registered by a plugin file appears in its
     * seat the moment it is registered, with no per-seat list to keep in step. */
    async open(host, api, opts) {
      this._api = api || {};
      this._seat = (opts && opts.seat) || "";
      /* THE HOST IS REMEMBERED BECAUSE THE BUILDER HAS TO COME BACK TO IT.
         openWorkflow used to render into #studio-body by id — correct while
         Studio was the only screen that mounted this, and wrong the moment a
         seat did: clicking a template in the seat's Generate tab found no such
         element and the builder opened nowhere at all. */
    ...
    _renderLibrary() {
      const body = this._libBody(); if (!body) return;
      const only = SEAT_CATS[this._seat || ""] || null;
      const byCat = {};
      this.templates
        .filter(t => !only || only.indexOf(t.category) !== -1)
        .forEach(t => (byCat[t.category] = byCat[t.category] || []).push(t));
      ...
      /* Saved workflows filter by the same rule: one saved under `audio` is the
         audio seat's, and showing it on art would make "your saved workflows"
         mean something different in every tab. */
      const savedShown = only
        ? this._saved.filter(w => only.indexOf(w.category) !== -1)
        : this._saved;
      if (savedShown.length) {
        html += sec("note", "Your saved workflows", savedShown.length,
          `<div class="wf-card-grid">${savedShown.map(s => `<button class="wf-card" onclick="WF.openSaved('${esc(s.id)}')"><span class="wf-card-g">${wfIcon(CAT_ICON[s.category] || "note", 18)}</span><span class="wf-card-t">${esc(s.name)}</span><span class="wf-card-h">${esc(s.category || "workflow")} · ${(s.stepCount || 0)} steps</span><span class="wf-card-x" onclick="event.stopPropagation();WF.deleteSaved('${esc(s.id)}')">✕</span></button>`).join("")}</div>`);
      }
    ...
    /* Back to the library, in the host this instance is mounted in. Inside a
       seat that is the seat's own tab; on Studio it is Studio's body, and the
       screen's own selector is used there so its subnav stays in agreement
       about which tab is showing. */
    backToLibrary() {
      if (this._seat && this._host) { this.open(this._host, this._api, { seat: this._seat }); return; }
      if (window.Studio && typeof window.Studio.select === "function") {
        window.Studio.select("workflows"); return;
      }
      if (this._host) this.open(this._host, this._api, {});
    },

    /* ---- builder -------------------------------------------------------- */
    openWorkflow(wf) {
      this._wf = wf;
      const host = this._host || document.getElementById("studio-body");
      if (!host) return;
      host.innerHTML = `<div class="wf-build">
        <div class="wf-top">
          <button class="qbtn small ghost" onclick="WF.backToLibrary()">← library</button>
          ...
          ${wf.fromTemplate && !wf.adopted
            ? `<span class="wf-scratch" title="Edits are kept so a run can find them, but this stays the shipped template until you save your own copy.">shipped template</span>
               <button class="qbtn small" onclick="WF.adopt()">save as mine</button>`
    ...
    _serialize() {
      ...
      /* `fromTemplate` and `adopted` RIDE ALONG. They decide whether this
         workflow belongs in your saved index, and a field dropped here comes
         back false on the next open — so an adopted copy would revert to a
         shipped-template scratchpad after a reload and stop saving itself. */
      return { id: this._wf.id, name: this._wf.name, category: this._wf.category,
               fromTemplate: this._wf.fromTemplate || "",
               adopted: !!this._wf.adopted,
    ...
    /* THE WRITE IS CHECKED. The document store answers 409 on a stale write (two
       ...
       storm. */
    /* A TEMPLATE SCRATCHPAD IS NOT ONE OF YOUR WORKFLOWS UNTIL YOU SAY SO.
     *
     * THE BUG THIS FIXES. `persist()` runs on every graph mutation — including
     * dragging a node one pixel — and `save()` unconditionally pushed the
     * workflow into the saved index. So opening a shipped template and moving
     * anything filed it under "Your saved workflows", instantly and silently.
     * Six such rows (`wf_tpl_*`) were sitting in this project's index, none of
     * them saved by a decision anybody made, and the library they cluttered is
     * the one place a person looks for the workflows they actually wrote.
     *
     * THE DOCUMENT IS STILL WRITTEN, and it has to be: the scratchpad's stable
     * id is what lets a RUN started from a template keep finding itself across
     * a reload (see openTemplate). Losing that would trade a visible annoyance
     * for an invisible one — a run that polls nothing and reports "running"
     * forever. So the split is exactly: the document autosaves, the INDEX does
     * not, until `adopt()` is called by the explicit Save button.
     */
    async save(silent) {
      ...
      /* Unadopted template scratchpad: document only, index untouched.
         KEYED ON THE ID SHAPE, not on `fromTemplate` alone. openTemplate gives
         the scratchpad a stable `wf_tpl_<template>` id and nothing else ever
         has one, so this is exact — and it does the right thing for copies
         saved before `adopted` existed, which carry `fromTemplate` and a random
         id and are genuinely the user's. */
      const scratch = String(wf.id || "").startsWith("wf_tpl_") && !this._wf.adopted;
      if (!scratch) {
      ...
      if (!silent) {
        toast(scratch
          ? "this is still the shipped template — press “save as mine” to keep your version"
          : "workflow saved");
      }
    },

    /* Take a copy of the template as YOUR workflow. A new id, because the
       scratchpad id belongs to the template and re-opening the template must
       not reopen your edits — and because two people adopting the same template
       would otherwise write over each other's copy. */
    async adopt() {
      if (!this._wf) return;
      const from = this._wf.fromTemplate || "";
      this._wf = { ...this._wf, id: uid("wf"), adopted: true,
                   name: this._wf.name + " (mine)", fromTemplate: from };
      await this.save(false);
      this.openWorkflow(this._wf);   // redraw so the header shows the new name
      this._renderLibrary();
    },
