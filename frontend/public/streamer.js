...
(function () {
  ...
  function mount() {
    // THE BELL IS THE ANCHOR, NOT THE STATUS BAR. This looked the chip's home
    // up as `.statusbar` and then inserted before `#nt-host` — two elements
    // that were siblings until the 4a shell moved the bell into its header.
    // The old bar is still in the document (hidden), so the guard passed and
    // insertBefore threw NotFoundError on every load: the streamer chip simply
    // never appeared, and the error was the only sign.
    var bell = document.getElementById('nt-host');
    var bar = (bell && bell.parentElement) || document.querySelector('.statusbar');
    if (!bar || host()) return;
    ...
    if (bell && bell.parentElement === bar) bar.insertBefore(el, bell);
