...
(function () {
  ...
  WF._renderLibrary = function () {
    /* Scoped to this mount's host — see WF._libBody in wf.js. A document-wide
       id lookup renders the seat's library into Studio's hidden copy. */
    var body = (this._libBody && this._libBody())
      || document.getElementById("wf-lib-body");
    if (!body) return;
    var self = this;

    /* SEAT SCOPING, HONOURED HERE TOO. wf.js's own _renderLibrary filters on
       WF._seat, and this function REPLACES it — so a filter applied only there
       is a filter that never runs, which is exactly what happened: the video
       seat's Generate tab opened showing the 2D asset family. The rule lives in
       wf.js (SEAT_CATS) and is read through the accessor so the two renders
       cannot drift on what a seat owns. */
    var only = (WF.seatCats && WF.seatCats(this._seat)) || null;
    var byCat = {};
    (this.templates || []).forEach(function (t) {
      if (only && only.indexOf(t.category) === -1) return;
    ...
    var savedShown = only
      ? (this._saved || []).filter(function (w) { return only.indexOf(w.category) !== -1; })
      : (this._saved || []);
    if (this._savedError) {
      html += panel(savedFam,
      ...
      html += panel(savedFam, savedShown.map(function (s) {
    ...
    build: function (facts) {
      ...
      return {
        nodes: [
          ...
          { id: "gate", type: "control.gate", x: X(2), y: Y },
          // The sheet describes itself: tileset_describe writes the atlas
          // coordinates and layouts into <tileset>.tiles.json ONCE, and the
          // generate step reads them off disk. Prefilling them on the
          // generate step was the same handoff-by-copy the sidecar replaced.
          { id: "describe", type: "tool.tileset.describe", x: X(3), y: Y,
            config: { tileset: ts ? ts.res : "",
  ...
  try { if (WF._libBody && WF._libBody()) WF._renderLibrary(); } catch (e) {}
})();
