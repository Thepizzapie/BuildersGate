...
(function () {
  ...
  class NodeCanvas {
    ...
    _bindMinimap() {
      ...
      this._mapUp = () => { this._map = null; };
      window.addEventListener("mouseup", this._mapUp);
    }

    /** Let go of the window and the host. Call it before dropping the instance.
     *
     * Every listener this class puts on `window` is removed here. The two
     * self-removing guards elsewhere (`_mapMove`, `_onKey` check
     * host.isConnected) only fire if something moves the mouse or presses a key
     * AFTER the host detaches — they are a safety net for a leak, not a
     * teardown, and they never run at all on a canvas nobody touches again.
     *
     * The host's innerHTML is cleared too: a detached NodeCanvas that gets
     * re-mounted into the same element would otherwise paint on top of its own
     * previous DOM.
     */
    destroy() {
      window.removeEventListener("mousemove", this._winMove);
      window.removeEventListener("mouseup", this._winUp);
      window.removeEventListener("mousemove", this._mapMove);
      window.removeEventListener("mouseup", this._mapUp);
      window.removeEventListener("keydown", this._onKey);
      if (this._raf) cancelAnimationFrame(this._raf);
      if (this._mraf) cancelAnimationFrame(this._mraf);
      this._raf = this._mraf = null;
      try {
        if (this.host) { this.host.classList.remove("nc-host"); this.host.innerHTML = ""; }
      } catch (e) {}
      this.host = null;
    }
    ...
    _bindCanvas() {
      ...
      // NAMED, not inline closures, because destroy() has to be able to take
      // them off again. An anonymous handler on `window` outlives its canvas
      // forever: a host that gets unmounted and remounted (the console's
      // board/graph switch does exactly that) left one live mousemove listener
      // per visit, each holding a whole detached graph.
      this._winMove = (e) => this._onMove(e);
